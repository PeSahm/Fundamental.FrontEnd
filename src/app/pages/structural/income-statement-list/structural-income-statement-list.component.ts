...
interface StructuralIncomeStatementItem {
  id: string;
  symbol: string;
  isin: string;
  fiscalYear: number;
...
interface StructuralIncomeStatementDetailItem {
  row: number;
  codalRow: number;
  description: string;
  value: number | null;
...
@Component({
  selector: 'app-structural-income-statement-list',
  templateUrl: './structural-income-statement-list.component.html',
  styleUrls: ['./structural-income-statement-list.component.scss']
})
export class StructuralIncomeStatementListComponent implements OnInit, OnDestroy {
  selectedItems: any = [];
  fiscalYear: number | null = null;
  reportMonth: number | null = null;
  reportFilter = { pageSize: 20, pageNumber: 1, orderBy: '' };
  totalRecords: number = 0;
  reports: StructuralIncomeStatementItem[] | null = null;
  children: StructuralIncomeStatementDetailItem[] | null = null;
  isLoading = true;
  ...
  constructor(private service: StructuralIncomeStatementService) { }
  ...
  getDetailRow(row: DetailRow<StructuralIncomeStatementItem>) {
    if (row.expand) {
