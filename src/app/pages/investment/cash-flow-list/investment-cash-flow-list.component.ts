...
interface InvestmentCashFlowItem {
  id: string;
  symbol: string;
  isin: string;
  fiscalYear: number;
...
interface InvestmentCashFlowDetailItem {
  row: number;
  codalRow: number;
  description: string;
  value: number | null;
...
@Component({
  selector: 'app-investment-cash-flow-list',
  templateUrl: './investment-cash-flow-list.component.html',
  styleUrls: ['./investment-cash-flow-list.component.scss']
})
export class InvestmentCashFlowListComponent implements OnInit, OnDestroy {
  selectedItems: any = [];
  ...
  reports: InvestmentCashFlowItem[] | null = null;
  children: InvestmentCashFlowDetailItem[] | null = null;
  isLoading = true;
  ...
  constructor(private service: InvestmentCashFlowService) { }
  ...
  getDetailRow(row: DetailRow<InvestmentCashFlowItem>) {
    if (row.expand) {
