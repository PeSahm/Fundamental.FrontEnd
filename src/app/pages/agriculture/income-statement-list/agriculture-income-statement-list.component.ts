...
interface AgricultureIncomeStatementItem {
  id: string;
  symbol: string;
  isin: string;
  fiscalYear: number;
...
interface AgricultureIncomeStatementDetailItem {
  row: number;
  codalRow: number;
  description: string;
  value: number | null;
...
@Component({
  selector: 'app-agriculture-income-statement-list',
  templateUrl: './agriculture-income-statement-list.component.html',
  styleUrls: ['./agriculture-income-statement-list.component.scss']
})
export class AgricultureIncomeStatementListComponent implements OnInit, OnDestroy {
  selectedItems: any = [];
  fiscalYear: number | null = null;
  reportMonth: number | null = null;
  reportFilter = { pageSize: 20, pageNumber: 1, orderBy: '' };
  totalRecords: number = 0;
  reports: AgricultureIncomeStatementItem[] | null = null;
  children: AgricultureIncomeStatementDetailItem[] | null = null;
  isLoading = true;
  ...
  constructor(private service: AgricultureIncomeStatementService) { }
  ...
  getDetailRow(row: DetailRow<AgricultureIncomeStatementItem>) {
    if (row.expand) {
