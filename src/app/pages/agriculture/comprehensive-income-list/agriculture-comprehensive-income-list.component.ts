...
import { AgricultureComprehensiveIncomeListItem } from 'src/app/models/agriculture/comprehensive-income.model';

@Component({
  selector: 'app-agriculture-comprehensive-income-list',
  templateUrl: './agriculture-comprehensive-income-list.component.html',
  styleUrls: ['./agriculture-comprehensive-income-list.component.scss']
})
export class AgricultureComprehensiveIncomeListComponent implements OnInit, OnDestroy {
  selectedItems: any[] = [];
  fiscalYear: number | null = null;
  reportMonth: number | null = null;
  reportFilter = { pageSize: 20, pageNumber: 1, orderBy: '' };
  totalRecords = 0;
  reports: AgricultureComprehensiveIncomeListItem[] = [];
  isLoading = true;
  ...
  constructor(private service: AgricultureComprehensiveIncomeService, private router: Router) { }
  ...
  openViewPage(rowItem: AgricultureComprehensiveIncomeListItem): void { if (rowItem?.id) { this.router.navigate(['/agriculture/comprehensive-income', rowItem.id]); } }
