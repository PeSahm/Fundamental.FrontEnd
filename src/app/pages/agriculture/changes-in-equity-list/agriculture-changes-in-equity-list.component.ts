...
import { AgricultureChangesInEquityListItem } from 'src/app/models/agriculture/changes-in-equity.model';

@Component({
  selector: 'app-agriculture-changes-in-equity-list',
  templateUrl: './agriculture-changes-in-equity-list.component.html',
  styleUrls: ['./agriculture-changes-in-equity-list.component.scss']
})
export class AgricultureChangesInEquityListComponent implements OnInit, OnDestroy {
  selectedItems: any[] = [];
  fiscalYear: number | null = null;
  reportMonth: number | null = null;
  reportFilter = { pageSize: 20, pageNumber: 1, orderBy: '' };
  totalRecords = 0;
  reports: AgricultureChangesInEquityListItem[] = [];
  isLoading = true;
  ...
  constructor(private service: AgricultureChangesInEquityService, private router: Router) { }
  ...
  openViewPage(rowItem: AgricultureChangesInEquityListItem): void { if (rowItem?.id) { this.router.navigate(['/agriculture/changes-in-equity', rowItem.id]); } }
