...
interface ServicesIndustryCashFlowItem {
  id: string;
  symbol: string;
  isin: string;
  fiscalYear: number;
...
interface ServicesIndustryCashFlowDetailItem {
  row: number;
  codalRow: number;
  description: string;
  value: number | null;
...
@Component({
  selector: 'app-services-industry-cash-flow-list',
  templateUrl: './services-industry-cash-flow-list.component.html',
  styleUrls: ['./services-industry-cash-flow-list.component.scss']
})
export class ServicesIndustryCashFlowListComponent implements OnInit, OnDestroy {
  selectedItems: any = [];
  fiscalYear: number | null = null;
  reportMonth: number | null = null;
  reportFilter = { pageSize: 20, pageNumber: 1, orderBy: '' };
  totalRecords: number = 0;
  reports: ServicesIndustryCashFlowItem[] | null = null;
  children: ServicesIndustryCashFlowDetailItem[] | null = null;
  isLoading = true;
  ...
  constructor(private service: ServicesIndustryCashFlowService) { }
  ...
  getDetailRow(row: DetailRow<ServicesIndustryCashFlowItem>) {
    if (row.expand) {
