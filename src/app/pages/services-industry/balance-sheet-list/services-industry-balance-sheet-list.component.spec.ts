...
describe('ServicesIndustryBalanceSheetListComponent', () => {
  ...
  let serviceSpy: jasmine.SpyObj<ServicesIndustryBalanceSheetService>;

  const mockListResponse = {
    data: {
      items: [
        { id: 'bs-1', symbol: 'وخارزم', isin: 'IRO1KHRZ0001', fiscalYear: 1402, reportMonth: 6, publishDate: '1402/07/01', version: 6, uri: 'https://codal.ir/1' }
      ],
      meta: { total: 1 }
    }
  };
  ...
  beforeEach(async () => {
    serviceSpy = jasmine.createSpyObj('ServicesIndustryBalanceSheetService', ['getAll', 'getById']);
    ...
    await TestBed.configureTestingModule({
      ...
      providers: [
        { provide: ServicesIndustryBalanceSheetService, useValue: serviceSpy },
        provideNgxMask(),
      ]
    }).compileComponents();
  ...
  it('getDetailRow should call getById and set children when expanding', () => {
    const fakeRow = { id: 'bs-1', symbol: 'وخارزم', isin: 'IRO1KHRZ0001', fiscalYear: 1402, reportMonth: 6, publishDate: '1402/07/01', version: 6, uri: 'https://codal.ir/1' };
    component.getDetailRow({ expand: true, rowData: fakeRow });
    expect(serviceSpy.getById).toHaveBeenCalledWith('bs-1');
