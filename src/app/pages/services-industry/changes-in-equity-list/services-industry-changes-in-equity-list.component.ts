...
import { ServicesIndustryChangesInEquityListItem } from 'src/app/models/services-industry/changes-in-equity.model';

@Component({
  selector: 'app-services-industry-changes-in-equity-list',
  templateUrl: './services-industry-changes-in-equity-list.component.html',
  styleUrls: ['./services-industry-changes-in-equity-list.component.scss']
})
export class ServicesIndustryChangesInEquityListComponent implements OnInit, OnDestroy {
  selectedItems: any[] = [];
  fiscalYear: number | null = null;
  reportMonth: number | null = null;
  reportFilter = { pageSize: 20, pageNumber: 1, orderBy: '' };
  totalRecords = 0;
  reports: ServicesIndustryChangesInEquityListItem[] = [];
  isLoading = true;
  ...
  constructor(private service: ServicesIndustryChangesInEquityService, private router: Router) { }
  ...
  openViewPage(rowItem: ServicesIndustryChangesInEquityListItem): void { if (rowItem?.id) { this.router.navigate(['/services-industry/changes-in-equity', rowItem.id]); } }
